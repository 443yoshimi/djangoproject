...
class salesRClass:
    ...
    def srdata_sort(self,dt):
        # JANコード、JANコード2の昇順にソート
        sorted_data = sorted(dt, key=lambda x:(x[3], x[4]))
    ...
    def srdata_uniqsum(self,dt): 
        ...
        for i,row in enumerate(dt):
            ...
            if (gokeirow == '') and (rowlst[4] == ''):
                ...
                # 明細削除
                #  →　合計、明細の順にデータをならべているため、合計、明細が一致してしまう場合がある
                #　→　小区分が入ってない明細は削除する
                # 販売点数が0より大きい場合 かつ
                # 店舗(2)、JAN(3)、販売点数(6)、販売金額(7)が重複していない場合 かつ
                # 小区分(12)が入っている場合
                if ((rowlst[6] > 0) and
                ((tencd != rowlst[2]) or (jancd2 != rowlst[3]) or (selsu != rowlst[6]) or (selgaku != rowlst[7])) and
                ((rowlst[12] != '0') and (rowlst[12] != ''))):
                    # リストに明細行を追加
                    loaddata1.append(salesRClass.syokbn_str(rowlst))
                    # 日付(0)、曜日コード(1)、店コード(2)、JAN1(3)、JAN2(4)、商品名(5)、販売数量(6)、販売金額(7)、
                    # 値引なし販売数(8)、値引1-30%販売数(9)、値引31%以上販売数(10)、
                    # 廃棄数(11)、小分類(12)、最終販売時刻コード(13)、大中小値段範囲(14)
    ...
    def srdata_cr(self):
        ...
        for row in self.loaddata3:
            ...
            # {graph:"100,0,0,0", hinnm:"りんご,0215456555551", bunrui:"大", saleskngsu:"24510,15", 
            ...
            self.srdictlist.append({
                'graph':graph,'hinnm':hinnm,'bunrui':bunrui,'saleskngsu':saleskngsu,
